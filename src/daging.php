		...
		<?php
			...
			if(!isset($_GET["sort"])|| !isset($_GET["order"])){
				$result = mysql_query("select * from catalog_product where category = 'daging' order by name Asc");
				$result1 = mysql_query("select path from catalog_product where category = 'daging' order by name Asc");
			}else{
				$result = mysql_query("select * from catalog_product where category = 'daging' order by ".$_GET["sort"]." ".$_GET["order"]);
				$result1 = mysql_query("select path from catalog_product where category = 'daging' order by ".$_GET["sort"]." ".$_GET["order"]);
			}
		...
		<div id="apa">
			...
			<script>
				function gantiUrutan(){
					...
					window.location.href = "daging.php?sort=" + a + "&order=" + b;
				}
