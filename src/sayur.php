		...
		<?php
			...
			if(!isset($_GET["sort"])|| !isset($_GET["order"])){
				$result = mysql_query("select * from catalog_product where category = 'sayur' order by name Asc");
				$result1 = mysql_query("select path from catalog_product where category = 'sayur' order by name Asc");
			}else{
				$result = mysql_query("select * from catalog_product where category = 'sayur' order by ".$_GET["sort"]." ".$_GET["order"]);
