		...
		<?php
			...
			if(!isset($_GET["sort"])|| !isset($_GET["order"])){
				$result = mysql_query("select * from catalog_product where category = 'alat tulis' order by name Asc");
				$result1 = mysql_query("select path from catalog_product where category = 'alat tulis' order by name Asc");
			}else{
				$result = mysql_query("select * from catalog_product where category = 'alat tulis' order by ".$_GET["sort"]." ".$_GET["order"]);
				$result1 = mysql_query("select path from catalog_product where category = 'alat tulis' order by ".$_GET["sort"]." ".$_GET["order"]);
			}
		...
		<div id="apa">
			...
			<script>
				function gantiUrutan(){
					...
					window.location.href = "beras.php?sort=" + a + "&order=" + b;
				}
				...
				if (xmlhttp.readyState==4 && xmlhttp.status==200)
					{
						if (parseInt(xmlhttp.responseText) >= parseInt(jumlah)) {
							...
							alert('Jumlah tersisa ='+xmlhttp.responseText);
						}
					}
				}
			}
			 ...
			 ?>
				...
				window.innerHeight) {
					...
					for(var i = page*10; (i < page*10+10 && i < b); i++){
						...
						s +=	'Jumlah Pembelian: <input type="text" name="jumlah" id="jumlah'+id[i]+'">'+ "<br/>";
